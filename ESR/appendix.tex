...
We now provide a full justification for the conditioning step yielding equation (\ref{conditionalDistnFixed_f}) in Section \textcolor{blue}{\ref{general_framework}}. The justification essentially follows from Theorem 3.3 in \textcolor{blue}{\citep{owhadi2015conditioning}}. This theorem is given below (we only give the part of the theorem we require): \vspace{5pt}
...
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
is already written in its $(\mathcal{H}_{1},\mathcal{H}_{2})$ partition representation. In the section on shorted operators in \textcolor{blue}{\citep{owhadi2015conditioning}} it is stated that provided the lower right partition of the covariance operator of (\ref{jointInfoDist}) corresponding to the covariance in $\mathcal{H}_{2}$, i.e. $\mathcal{I}\mathcal{L}V(\mathcal{I}\mathcal{L})^{*}$, is invertible then the shorted operator is given by:
\begin{equation*}
    \begin{pmatrix}
    ...
    \end{pmatrix}
\end{equation*}
We assume the invertibility of this partition. \vspace{5pt}
